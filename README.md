...
## Проблема
Оригинальный счётчик весов конвейера добавляет в аккумулятор (DINT) 1 тонну по каждому дискретному импульсу от весов. При попытке перевести аккумулятор в REAL и добавлять по 0.83 тонны каждый импульс возникла проблема с вещественными числами с плавающей точкой:
